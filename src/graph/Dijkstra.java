...
public class Dijkstra
 {
    ...
    // we use this because in the priority queue, the pairs will come in and we need
    // to arrange them in ascending order
    {
        ...
        @Override
        // this is for comparing in ascending order, for descending order we can reverse
    ...
    public static List shortestPath(ArrayList<edge>[] graph, node[] nodes, int src, int dest) {
        ...
        Arrays.fill(parent, -1);//no parent initally

        if (nodes[src].blocked) // source is blocked
        {
            System.out.println("Source blocked");//u lose
