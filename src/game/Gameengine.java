...
public class Gameengine {
    ...
    public MoveResult movePrisonerStep() {
        ...
        if (idx >= path.size() - 1) {
            return pLoc == target ? MoveResult.EXIT_REACHED : MoveResult.NO_MORE_STEPS;
        }
    ...
    private void validateSetup(int nodes, int edgeCount, int src, int dst) {
        if (nodes < 2) {
            throw new IllegalArgumentException("ateast 2 nodes are required");
        }
        if (!isValidNode(src, nodes) || !isValidNode(dst, nodes)) {
    ...
    private void generateGridGraph(int targetEdgeCount) {
        ...
        // add more random edges until we reach target
        int added = rooms - 1;
    ...
    private List<Integer> buildCurrentPath() {
        ...
        for (int i = 0; i < rooms; i++) {
            ...
            for (int[] neighbor : g.adj.get(i)) {
                if (!isEdgeBlocked(i, neighbor[0])) {
                    graphData[i].add(new edge(i, neighbor[0], neighbor[1]));
                }
            }
        }
